...
from libs.utils.load_vit_weights import *
...
parser.add_argument('--sa_drop_path_rates', default=[0.,0.,0.], nargs='+', type=float, help='drop_path rate in self attention on spatial,structured,temporal cache')
parser.add_argument('--sa_dropout_rates', default=[0.1,0.1,0.1], nargs='+', type=float, help='dropout in self attention layers on spatial,structured,temporal cache')
...
parser.add_argument('--sa_n_layers', default=[6,6,6], nargs='+', type=int, help='number of cca self attention layers for spatial,structured,temporal cache.')
parser.add_argument('--sa_n_heads', default=[8,8,8], nargs='+', type=int, help='number of cca self attention heads for spatial,structured,temporal cache.')
...
parser.add_argument('--learnable_patch_pos', action='store_true', help='true if use learnable position embeddings in self attn')
...
parser.add_argument('--per_pixel_mse', action='store_true', help='true if calculate mse per pixel')
...
def set_config(config, conf_type='default'):
    ...
    if 'cca' in conf_type:
        ...
        config[0]['sa_drop_path_rates'] = args.sa_drop_path_rates
        config[0]['sa_dropout_rates'] = args.sa_dropout_rates
...
def train(shared_model, task, batch_size, train_steps, gpu_id, start,  restore,  counter, barrier=None, save_interval=None,
    ...
    for i in range(start+1, train_steps):
        ...
        elif task == 'vqa' or task == 'vqa_metric' or task == 'vqa_with_test':
            if (log and eval_interval is not None and i % eval_interval == 0 and i >= args.eval_start):
                if i == (start+1) and not eval_first:
                    continue
                model = model.eval()
...
if __name__ == '__main__':
    ...
    if args.restore_last == True:
        ...
        iters = [int(os.path.basename(c)) for c in ckpts if os.path.basename(c) not in ['best','res', 'val_res', 'test_res']]
        if len(iters) != 0:
            restore = max(iters)
        else:
            restore = -1
